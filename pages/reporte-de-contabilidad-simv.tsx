...
function ReporteSIMVComponents() {
...
const [ code, setCode ] = useState('CVCID');
...
const handleFileUpload = async () => {
  ...
  try {
    ...
    let nameValidFile =  `api.simv.gob.doSRTPapiSendxml{${token}}{${code}} - ${year}${month}`
    if (code === 'CVCID') {
     nameValidFile =  `api.simv.gob.doSRTPapiSendxml{${token}}{${code}} - ${year}${month}${day}`
    }
...
return (
  <>
    <div className='p-10'>
       <NamePage
          title={"Reporte de contabilidad SIMV"}
              ...
              <div className={'grid grid-cols-2 gap-4'}>
                <div>
                    <input 
                      ...
                      name={'dateSearch'}
                      type="date" 
                      className="border border-gray-300 text-gray-900 text-sm rounded focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 py-4 dark:focus:border-blue-500" placeholder="Select date" />
                </div>
