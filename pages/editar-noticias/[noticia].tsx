...
function EditNewsComponents() {
    ...
    const { updatedNews, getAllNews } = useNews();
    ...
    useEffect(() => {
        async function fetchData() {
          ...
          if(!!noticia && !isEmpty(news)) {
            setNewspaper(news.filter((item: any) => +item.id === +noticia));
        }
    ...
    useEffect(() => {
        if(get(authLogin, 'id', false) && !isEmpty(authLogin.permissions)) {
            const verify = VerificatePermissions(get(authLogin, 'permissions', []), [Permissions.NOTICIAS, Permissions.SUPER_ADMINISTRADOR]);
            if(!verify) {
                router.push('/');
            }
        }
    ...
    useEffect(() => {
        if(!isEmpty(newspaper)) {
            setInfo({
                title: newspaper[0].title,
                description: newspaper[0].description,
                category: newspaper[0].category,
            }); 
            setValue(newspaper[0].content);
        }
    }, [newspaper])
    ...
    const onUpdateNews = async (frontPageUrl?: string) => {
        setPreloading(true);
        const data: any = {
            ...info,
            content: value,
            frontPage: frontPageUrl ? frontPageUrl : get(newspaper[0], 'frontPage', ''),
            createdBy: authLogin.id,
            updatedBy: authLogin.id,
        };

        await updatedNews(data, noticia);
        setFile(null);
        setPreviewURL(null);
        setPreloading(false);
        ...
        try {
            ...
            return new Promise((resolve: any, reject: any) => {
                reader.onloadend = async () => {
                    ...
                    try {
                        const infoImage = await axios.post(`/api/azureBlobStorage?type=${get(file, 'type', '')}`, { base64Image });
    
                        if (infoImage.data.urlImage) {
                            setFrontPage(infoImage.data.urlImage);
                            console.log('aqui', infoImage.data.urlImage);
                            await onUpdateNews(infoImage.data.urlImage);
                            console.log('aqui 2', frontPage);
                            resolve();
                        } else {
                            reject(new Error('No se recibió ninguna URL de imagen.'));
                        }
                    } catch (error) {
                        reject(error);
                    }
                                            ...
                                            <div>
                                                {(previewURL || newspaper) &&  (
                                                    <>
                                                        ...
                                                        <div className="mt-2">
                                                            <Image
                                                                src={previewURL ? previewURL : get(newspaper[0], 'frontPage', '')}
                                                                width={1080}
                                                                height={1080}
                                                                className={"w-40 h-40 border rounded mt-2"}
                                                                alt={"logo de parval"}
                                                                priority
                                                            />
                                                        </div>
